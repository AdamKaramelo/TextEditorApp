...
public abstract class Document {
	...
	protected int countSyllables(String word){
		...
		if(chars.length == 1 && vowels.indexOf(chars[0]) != -1) {
			count++;
			return count;
		}
		...
		for(int i = 0; i < chars.length; i++) {
			if(i < chars.length-1) {
				if(vowels.indexOf(chars[i]) != -1) { //its a vowel
				...
				if(vowels.indexOf(chars[i]) != -1) { //last letter is vowel
					if(chars[i] == 'e') {
						if(count == 0) {
							count++;
							break;
						}else if(vowels.indexOf(chars[i-1]) == -1) { // pre of 'e' is not vowel (lone e)
							break;
						}else {  //pre of 'e' is vowel
							count++;
							break;
						}
					}else { //predecesor of last letter is vowel too 
						count++;
						break;
					}
				}
	...
	public double getFleschScore()
	{
		...
		double FleschScore = 206.835 - 1.015*words/sentences - 84.6*syll/words;
		return FleschScore;
	}
