...
/**
 * Class that suggests close words to the spelled word.
 *
 */
public class NearbyWords implements SpellingSuggest {
	...
	/** A method that provides the number of spelling suggestions for a misspelled word.
	...
	public List<String> suggestions(String word, int numSuggestions) {
		...
		queue.add(word);
		visited.add(word);
		while(!queue.isEmpty() && count < numSuggestions && searches <= THRESHOLD) {
			String curr = queue.remove(0);
			substitution(curr, queue, true);
			insertions(curr, queue, true);
			deletions(curr, queue, true);
			for(String n : queue) {
